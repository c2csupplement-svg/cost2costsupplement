...
const productSummary = useMemo(
  () => (productsData?.products || []).find((item) => item.slug === slug),
  [productsData, slug]
);

const { data: productData, isLoading: isProductLoading, isError: isProductError } =
  useGetProductBySlugQuery(slug, { skip: !slug });
const { data: relatedProductsData, isLoading: isRelatedLoading, isError: isRelatedError } =
    useGetRelatedProductsQuery(slug);
// The product API only returns brandId (no embedded brand object),
// so we look the brand name up from the brands list, same as ShopPage.
...
const flavours = useMemo(
  () => [...new Set(variants.map((v) => v.flavour).filter(Boolean))],
  [variants]
);
...
const sizes = useMemo(
  () => [...new Set(
    variants
      .filter((v) => !selectedFlavour || v.flavour === selectedFlavour)
      .map((v) => v.size)
      .filter(Boolean)
  )],
  [variants, selectedFlavour]
);

useEffect(() => {
  if (flavours.length && !flavours.includes(selectedFlavour)) setSelectedFlavour(flavours[0]);
...
const selectedVariant =
  variants.find(
    (v) =>
      (!selectedFlavour || v.flavour === selectedFlavour) &&
      (!selectedSize || v.size === selectedSize)
  ) || variants[0];

const product = useMemo(() => {
  if (!apiProduct) return null;
  const priceValue = selectedVariant?.discountedPrice ?? selectedVariant?.price ?? null;
  const originalValue = selectedVariant?.discountedPrice
    ? selectedVariant?.price
    : null;
  const apiImages = [
    ...(Array.isArray(apiProduct.images) ? apiProduct.images : []),
    ...(apiProduct.featuredimg ? [apiProduct.featuredimg] : []),
  ...
  return {
    ...
    brand:
      ...
      brandsById[apiProduct.brandId] ||
      "Cost2Cost",
    category: apiProduct.category?.name || "Uncategorized",
    images: apiImages,
    price: priceValue !== null ? Number(priceValue) : null,
    originalPrice: originalValue !== null ? Number(originalValue) : null,
    flavours,
    inStock: (selectedVariant?.stockQuantity ?? 0) > 0,
    reviewCount: apiProduct._count?.reviews ?? apiProduct.reviews?.length ?? 0,
    selectedVariant,
  };
}, [apiProduct, selectedVariant, flavours, brandsById]);

useEffect(() => {
  setSelectedImage(0);
}, [apiProduct?.id, selectedVariant?.id]);
...
if (isProductsLoading || (productSummary && isProductLoading)) {
  return <><Header /><main className="min-h-[70vh] flex items-center justify-center"><p>Loading product...</p></main><Footer /></>;
}

if (isProductsError || isProductError) {
  return <><Header /><main className="min-h-[70vh] flex items-center justify-center"><p>Unable to load product.</p></main><Footer /></>;
}
...
return (
    ...
    <div className="mt-6 flex flex-col gap-3">
        ...
        {/* ================= RELATED PRODUCTS ================= */}
          ...
          {!isRelatedLoading && relatedProductsData.length > 0 && (
            <ProductSlider
