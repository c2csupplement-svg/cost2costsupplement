...
function normalizeProductList(value) {
  ...
  if (Array.isArray(value?.data?.relatedProducts)) {
...
export default function ProductDetailsPage({
  ...
  const routeSlug = Array.isArray(params?.slug)
    ? params.slug[0]
    : params?.slug;
  ...
  const [relatedProductsData, setRelatedProductsData] =
    useState([]);
  ...
  variants.forEach((variant) => {
    getVariantAttributes(
      ...
      if (!variantAttributeGroups[key]) {
        variantAttributeGroups[key] = {
          name: attribute.name,
          slug: attribute.slug,
          values: [],
        };
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:items-start lg:gap-12">
              <div className="min-w-0 lg:sticky lg:top-24 lg:self-start">
                <div className="grid gap-4 md:grid-cols-[90px_minmax(0,1fr)]">
                  <div className="order-2 flex w-full gap-3 overflow-x-auto pb-2 scrollbar-hide md:order-1 md:w-[104px] md:min-w-[104px] md:max-h-[600px] md:flex-col md:overflow-x-hidden md:overflow-y-auto md:pr-2">
                    {product.images.map(
                      (
                        image,
                        index
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 md:mt-7 flex flex-row gap-3">
                  ...
                  <button
                    ...
                    className={`flex cursor-pointer h-10 flex-1 items-center justify-center gap-2 rounded-lg border text-sm font-bold transition sm:h-14 ${wishlistActive
            ...
            {isRelatedLoading && (
              <section className="border-t border-[#E5E5E5] bg-white mt-10">
                <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10">
                  <div className="mb-5">
                    <div className="skeleton-shimmer h-3 w-28 rounded bg-[#EDEDED]" />
                    <div className="skeleton-shimmer mt-2 h-8 w-56 rounded bg-[#EDEDED]" />
                  </div>

                  <div className="flex gap-4 overflow-x-auto pb-1">
                    {[1, 2, 3, 4].map((item) => (
                      <div key={item} className="min-w-[220px] sm:min-w-[280px]">
                        <SkeletonProductCard />
                      </div>
                    ))}
                  </div>
